...
//corsproxy gets around the fact that the api headers don't allow CORS requests
// :'(
...
function Cqc(timeToShow) {
  ...
  this.showNotifications = function(details, monthsOffset) {
    ...
    $.when.apply($, notificationDeferreds).then(loader.hide, function(failure) {
      loader.hide();
      document.getElementById("status").innerHTML = "Some points failed to load. <a href='limitations.html'>Read why</a>, or <a href='index.html'>try a mirror</a>";
    });
  };
  this.buttons = {
    elems: {
